(function(w, d) {
	...
	var _sN,
		...
		_settings = {
			...
			matchAll: false						// match all routes
		};
	...
	_.route = function(path, directions, title) {
		var params = [];
		path = '^' + path;
		...
		path += '\\/?$';
		_routes.push({ regex : new RegExp(path), params : params, directions : directions, title : title });
	};
	...
	_.change = function() {
		...
		if (route) {
			...
			var change = _.dispatch('change', _current, _params, old);
			var title = (typeof route.title === 'function') ? route.title(_params) : route.title;
			d.title = title;
			if (_settings.html5 && 'pushState' in w.history) {
				w.history.replaceState(_.extend(_params, { hash : _current }), title, '/' + _current);
			}
			var end = function() {
		...
		} else if (_settings.matchAll) {
			_.dispatch('change', _current);
			if (_settings.html5 && 'pushState' in w.history) {
				w.history.replaceState({ hash : _current }, d.title, '/' + _current);
			}
		} else {
	...
	_.match = function(h) {
		for (var r in _routes) {
			...
			if (matched) {
				...
				return { directions : route.directions, params : p, title: route.title };
			}
		}
		return undefined;
	};
	...
	_sN = function(config) {
		...
		if (_settings.matchAll) _.setup(_.change);
		return _sN;
	};
	...
	_sN.navigate = function(route) {
		if ('path' in route) {
			if (_routes.length === 0 && !_settings.matchAll) {
				_.setup(_.change);
			}
			_.route(_.sanitize(route.path), ('directions' in route) ? route.directions : function() {}, ('title' in route) ? route.title : d.title);
		} else {
